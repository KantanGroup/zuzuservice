...
public enum AppleCategoryEnum {
    ...
    private static final Map<String, AppleCategoryEnum> map;

    static {
        map = new HashMap<String, AppleCategoryEnum>();
        for (AppleCategoryEnum v : AppleCategoryEnum.values()) {
            map.put(v.getCategory() + "", v);
        }
    }

    public static AppleCategoryEnum findByKey(String id) {
        return map.get(id);
    }
}
