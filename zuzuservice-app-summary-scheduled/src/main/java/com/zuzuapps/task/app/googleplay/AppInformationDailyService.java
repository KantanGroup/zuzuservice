    ...
    private void processDailyAppInformation(File[] files) throws Exception {
        ...
        for (File json : files) {
            ...
            if (data.length >= 3) {
                ...
                try {
                    extractAppInformation(languageCode, appId, true);
                    moveFile(json.getAbsolutePath(), CommonUtils.folderBy(rootPath, DataServiceEnum.information_daily.name(), DataTypeEnum.log.name(), time, countryCode).getAbsolutePath());
                    ...
                    if (ex.getCode() == ExceptionCodes.NETWORK_LIMITED_EXCEPTION) {
                        ...
                        moveFile(json.getAbsolutePath(), CommonUtils.folderBy(rootPath, DataServiceEnum.information_daily.name(), DataTypeEnum.not_found.name(), time).getAbsolutePath());
                        logger.info("[Information Store][" + appId + "][" + languageCode + "]Error " + ex.getMessage());
                    } else {
                        moveFile(json.getAbsolutePath(), CommonUtils.folderBy(rootPath, DataServiceEnum.information_daily.name(), DataTypeEnum.error.name(), time).getAbsolutePath());
