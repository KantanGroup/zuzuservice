...
do
    ...
    link="http://api.zuzuapps.com/solr/app-trend-index/select?indent=on&q=*:*&rows=$size&start=$((size*page))&wt=json"
    #link="http://localhost:8983/solr/app-trend-index/select?indent=on&q=*:*&rows=$size&start=$((size*page))&wt=json"
    #echo $link
    curl $link | jq ".response.docs" | jq "map(del (._version_))" > $file_name
